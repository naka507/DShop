...
> 上文 22 项契约测试全绿、`npm run check` 全绿之后，仍对**真实部署**（`dshop-dev` D1 + 已发布 Worker）做了端到端验证。结果发现一个**测试替身结构上不可能暴露**的缺陷——记录在此，作为「单测全绿 ≠ 线上可用」的证据。
