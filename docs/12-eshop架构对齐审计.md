...
**「后台可见可重放」的落地端点（本版补齐，此前只是声称）**：`GET /api/v1/admin/task-queue`（列表，默认只列 `failed`）、`GET /api/v1/admin/task-queue/:id`（详情）、`POST /api/v1/admin/task-queue/:id/replay`（重放）。权限点 `task:dead_letter:manage`；重放为单条原子 UPDATE（守卫 `status='failed'` 在 SQL 的 `WHERE` 里），**类型未在本版本注册时先拒后写**。详见 §12.14 P0-4。
...
| **P0-4** | **死信「可重放」只是文档声称** | `docs/08:111` / `docs/12:317` 都写「置 `failed` **进死信，后台可见可重放**」，但 `apps/api/src/routes/admin/` 下**无任何 task-queue 入口**——死信只能人肉连 D1 改库，「可重放」是空头承诺 | ✅ **已闭环**：新增 `routes/admin/task-queue.ts`（列表 / 详情 / 重放三端点）+ `repositories/task-queue.ts`（`listTaskQueue` / `findTaskById` / `replayFailedTask`）。重放是**单条原子 UPDATE**（守卫 `status='failed'` 写在 SQL 的 `WHERE` 里，不是「先查后写」——并发下不会双重重放）；**类型未在本版本注册时先拒后写**（`ERR_ADMIN_TASK_TYPE_UNREGISTERED`，判据是消费侧 `createTaskHandlers()` 的真实键集合，不另立类型清单），避免把跑不了的任务塞回队列。权限点 `task:dead_letter:manage`（超管自动持有，平台运营**不**持有）；重放落 `audit_logs`（`task_queue.replay`）。**20 项契约测试**（`tests/admin-task-queue.test.ts`，含「未知路径必须 404 而非 401/403」「畸形 payload 不 500」两条结构性负向控制、两条**白名单同源**护栏，以及**三端点各自的 401/403 负向控制**；复核后的二次修复见 §12.14.0）。⚠️ **过程中真实踩坑并已修正**：首版把 `status` 白名单**手抄**成 `pending/processing/succeeded/failed`，而消费侧落库的完成态是 `done`（`jobs/task-queue.ts:575` 的 `SET status = 'done'`，与 `TASK_QUEUE_STATUS.DONE` 一致；`succeeded` 只是 `TaskQueueConsumeResult` 的**计数字段名**）。后果是 `?status=done` 返回 400 —— 运维查「已完成任务」会拿到 400 并误读成「没有数据」，属**静默功能缺失**。现白名单改为 `Object.values(TASK_QUEUE_STATUS)` **与枚举同源**，并补两条护栏测试（逐值 200 + `?status=done` 可查）锁死 |

### 12.14.0 复核后的二次修复（对抗性复核发现的 3 处 P1 + 2 处 P3）

> 上表 P0-4 的首版实现经对抗性复核确认下列缺陷，**全部已修并配负向控制**。

| #        | 缺陷（复核确认，均非理论问题）                                                                                                                                                                        | 修复动作                                                                                                                                                                                                                                                                                                                                                                                                                                                | 反注入验证                                                         |
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| **P1-1** | **列表信封用了 `items`，违反 `docs/06:21` 的三组统一分页形状**。管理端唯一取页函数 `apps/admin/src/api/client.ts` 的 `getPage` 只认 `data.list`（`Array.isArray(data.list) ? ... : []`）→ 任何页面用 `getPage` 调本端点会得到「空表格 + 非零 total」的**静默丢数据** | 在 `packages/shared/src/contracts/admin.ts` 新增 `AdminTaskQueueListQuerySchema`（`status` 取 `TASK_QUEUE_STATUS`、分页字段**复用** `PageQuerySchema`）与 `AdminTaskQueueListSchema`（`pageResultSchema(AdminTaskQueueRowSchema)`，与 `ShopOrderListSchema` 同写法）+ `AdminTaskQueueRow` 类型；路由改为 import 共享 schema、**删除现场定义**（P2-2），响应改为 `list` 并像 `shop/orders.ts:410-416` 一样 `safeParse` 校验后返回（失败走既有 `ADMIN_ERROR_CODES.INTERNAL_ERROR`，**未新增错误码**）                                                                       | 无需反注入（形状由 `pageResultSchema` 唯一产出）                    |
| **P1-2** | **新权限点没同步进 `roles.permissions` 种子**：`0002_seed.sql` 的超管数组仍是 7 项、不含 `task:dead_letter:manage`，而该文件头声明「逐字照录 `rbac.ts`」。`/admin/me` 读 DB（`repositories/admin-users.ts`），`requirePermission` 读代码侧 `ROLE_PERMISSIONS` → **两个真相源漂移**：前端菜单（`apps/admin/src/layout/menu.ts` 按 `permission` 过滤）对所有人隐藏入口，后端 API 却放行 | ① 种子超管数组补 `task:dead_letter:manage` 并修正计数注释（7 → 8）；② `platform_operator` **保持不含**该权限并在种子注释里点明这是设计意图（死信重放会重触发业务副作用）；③ 前端夹具 `apps/admin/tests/helpers.tsx` 的 permissions 补该项；④ **新增防漂移测试**（`packages/db/tests/schema.test.ts`）：真实 SQLite 跑 `0002_seed.sql` 后断言超管 `permissions` 与 `ALL_PERMISSIONS` 逐项相等、每个内置角色的种子 ⊇ `ROLE_PERMISSIONS`、运营种子**不含**死信权限 | 见下「防漂移测试」                                                  |
| **P1-3** | **详情路由的鉴权没有负向控制**：`GET /task-queue/:id` 挂了 `requireAdminAuth()` + `requirePermission(...)`，但 20 个用例**无一**请求该路径；把两条中间件删掉测试**全绿**——「详情端点完全无鉴权」无法被发现，而详情返回的 `payload` 是**明文**                                  | 补两条用例：详情**不带 Cookie → 401**、详情**用 `platform_operator` → 403**（并断言 403 的 `code` 为 `ERR_ADMIN_PERMISSION_DENIED`）；401/403 两条用例都改为「列表 / 详情 / 重放三端点各自成立」                                                                                                                                                                                                                                                        | **实测**：删掉详情路由的两条中间件 → 2 条用例失败（`× 未登录…`、`× 已登录但角色权限不足…`）→ 从改前内容恢复（非 `git checkout`）→ 20/20 全绿 |
| **P2-1** | **重放路由的 `changes === 0` 分支不可达**：handler 写库前先做 TS 侧 `row.status !== "failed"` 判断，端点测试永远在那一行返回，「不谎报成功」的 SQL 分支在端点层**零覆盖**                                                    | **删除** TS 侧冗余前置判断，让**单条原子 UPDATE 的 `changes === 0` 成为「只有失败态可重放」的唯一权威**；调整后顺序 a. `findTaskById` → 404；b. 类型未注册 → 409（先判后写）；c. 原子 UPDATE → `changes === 0` → 409 `TASK_NOT_FAILED`；d. 审计；e. 回显。理由写入注释：守卫放 SQL 不放 TS 是本仓纪律；保留 TS 判断会引入 `findTaskById` 与 `replayFailedTask` 之间的 **TOCTOU 窗口**，并使 SQL 守卫不可被观测                                                                                                | **实测**：把 `WHERE ... AND status = 'failed'` 里的守卫删掉 → 3 条用例失败（含端点层 `pending → 409`）→ 恢复 → 20/20 全绿      |
| **P3**   | `routes/admin/task-queue.ts:65-66` 的注释被改坏（「而消费侧实际落库的完成态」重复两次、句子不成立）                                                                                                      | 该段注释随契约上移一并重写为通顺语句（现位于 `packages/shared/src/contracts/admin.ts` 的 `AdminTaskQueueListQuerySchema` 文档注释与 `routes/admin/task-queue.ts` 的说明块）                                                                                                                                                                                                                                                                                    | —                                                                  |
| **P3**   | 白名单用例名声称「不遗漏、**不多抄**」但只断言逐值 200（只证「不遗漏」）                                                                                                                                | 补真正的「不多抄」断言：`?status=succeeded`（不属于 `TASK_QUEUE_STATUS`）→ 400                                                                                                                                                                                                                                                                                                                                                                            | **实测**：把 `succeeded` 注入白名单 → 该用例失败 → 恢复 → 20/20 全绿 |
| **P3-4** | **默认列表按 `created_at` 排序无索引**：`repositories/task-queue.ts` 的 `ORDER BY created_at DESC` 无对应索引（`0001_init.sql` 只建了 `idx_task_queue_status(status, run_at)`）                          | **只登记不改**（`0001_init.sql` 已应用到已部署 D1，改它对既有库无效且污染已发布迁移）：**将来数据量上来需新增迁移补 `idx_task_queue_created_at(created_at DESC)`**。当前量级（死信默认只列 `failed` 且 `pageSize` 上限 100）不构成实际瓶颈，故列为 **P3**、留待新增迁移                                                                                                                            | —                                                                  |

### 12.14.1 ★ 真实部署验证发现的「仅线上」缺陷（测试替身看不见）

> 上文 20 项契约测试全绿、`npm run check` 全绿之后，仍对**真实部署**（`dshop-dev` D1 + 已发布 Worker）做了端到端验证。结果发现一个**测试替身结构上不可能暴露**的缺陷——记录在此，作为「单测全绿 ≠ 线上可用」的证据。

| #        | 缺陷（仅真实部署可复现）                                                                                                                                                                                                                                                                                                                                                                                              | 修复动作                                                                                                                                                                                                                                                                                                                                                                                                                    | 验证                                                                                                                       |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **P1-4** | **非 ISO 时间列会让「整页」列表 500，而详情正常**。本仓生产者写 ISO（`packages/services/src/task-queue.ts` 的 `toISOString()`），但 `task_queue` 的三个时间列是**裸 `TEXT`、无 `DEFAULT`**（`0001_init.sql:614-624`）。任何绕过本仓生产者的写入（手工 `wrangler d1 execute` 修数、外部工具、将来新写入点）会落成 SQLite 的 `datetime('now')` 形态 `2026-09-26 17:33:54`（空格、无毫秒、无 `Z`）。该形态**通不过** `IsoDateTimeSchema`，而列表对**整页** `safeParse` → **一行**格式不合规就让**整页** 500。**死信入口恰恰是用来查看「状态不正常」那批行的**：因一行异常而整表看不见，正好把最该被看到的行藏起来——与本文档「`payload` 原样返回不解析，避免损坏行让入口 500」是同一条设计意图。测试替身 `helpers/sqlite-d1.ts` 的插入全用严格 ISO，故**结构上无法**暴露此问题 | 在**读边界**统一归一（`repositories/task-queue.ts` 的 `normalizeTimestamp`/`normalizeRow`）：`T` 形态原样返回，`YYYY-MM-DD HH:MM:SS[.fff]` → `YYYY-MM-DDTHH:MM:SS.sssZ`（`datetime('now')` 本就是 UTC，补 `Z` 语义正确）。**不放宽对外契约**——API 对外恒为 ISO，前端与 `IsoDateTimeSchema` 都不必知道底层存的是哪种形态 | 补 2 条用例（列表 / 详情各一，插入 `datetime('now')` 形态）；**反注入实测**：去掉 `normalizeRow` → 列表用例失败（`expected 500 to be 200`）→ 恢复 → 22/22 全绿。**真实部署实测**：修复前该列表 500、详情 200；修复后列表 200 且该行 `runAt` 归一为 `2026-09-26T17:33:54.000Z` |

**真实部署端到端验证记录**（`https://dshop-api.eeshop.workers.dev`，超管账号 + 真实 `dshop-dev` D1）：

- 列表（默认 `failed`）/ 详情 / 重放三端点均返回真实数据；**信封为 `list`**（P1-1 修复在线上生效）。
- `?status=done` → 200（P1-1 的 `succeeded` 白名单缺陷在线上确认已修）；`?status=succeeded` → 400。
- 未知路径 `GET /api/v1/admin/task-queue/x/y` → **404**（非 401/403，逐路由中间件在线上成立）；无 Cookie → 401。
- 重放**已注册类型** → 200，回查 D1 `status` 由 `failed` 变 `pending`、`attempts` 归 0；随后 **Cron 消费者真的消费了它**（`last_error` 变为 `task_payload_invalid`，因测试 payload 指向不存在的订单）——证明重放不是「只改状态」。
- 重放**未注册类型**（`notify.send`）→ 409 `ERR_ADMIN_TASK_TYPE_UNREGISTERED`，且回查 D1 **状态未被改动**（先判后写成立）。
- 对已变 `pending` 的行再次重放 → 409 `ERR_ADMIN_TASK_NOT_FAILED`（SQL 守卫在线上成立）。
- `audit_logs` 出现 `action = "task_queue.replay"`、`target_type = "task_queue"`（审计在线上落库）。
- ⚠️ **过程中另发现一处既有缺口（非本次改动引入）**：线上 Worker **未设置任何 `JWT_SECRET` 等 secret**，导致 `/admin/login` 返回 500——即此前「已真实部署」只验证了公开只读端点（`/health`、商品列表），**后台登录链路从未在线上跑通**。本次已为 development 环境补上占位 secret 并跑通，验证后清理了临时账号与测试数据。
...
| P0-4 | 死信运维入口：新增 `routes/admin/task-queue.ts`（列表 / 详情 / 重放）+ `repositories/task-queue.ts`；重放用单条原子 UPDATE（守卫在 SQL 的 `WHERE`），类型未注册时先拒后写；`status` 白名单**与 `TASK_QUEUE_STATUS` 同源**而非手抄；列表信封 `list`（`AdminTaskQueueListSchema`，`docs/06:21`） | 单测：20 项契约（含并发重放只有一次 `changes=1`、未注册类型不改库、未知路径 404 非 401/403、白名单逐值合法 + `succeeded` 400、三端点各自的 401/403） |
