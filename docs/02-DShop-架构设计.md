...
> **文档状态**：评审稿 v1.0 ｜ **日期**：2026-09-20 ｜ **技术基线**：TypeScript + Cloudflare 全托管（Workers / D1 / KV / R2 / Cron Triggers，Queues 按需）
> **与 PiEcho 的关系**：DShop 是独立完整的商城平台，同时以**只读 HTTP 契约**（`/api/v1/agent/*`）为 PiEcho 智能客服系统提供订单、物流、商品规格、库存、售后进度等业务事实数据。
...
7. [★ 供 PiEcho 的只读 Agent API 契约](#7--供-piecho-的只读-agent-api-契约)
8. [核心业务流程](#8-核心业务流程)
9. [认证与权限](#9-认证与权限)
...
| 角色 | 说明 |
| --- | --- |
| **A：独立商城平台** | 自有 C 端商城（PC/H5 响应式）、平台后台、商户后台，覆盖商品、交易、支付、售后、营销、结算全链路，独立运营、独立部署、独立演进 |
| **B：PiEcho 的只读业务数据源** | 对外暴露 `/api/v1/agent/...` 只读契约，供客服 Agent 查询订单状态、物流、商品规格、库存、售后进度、售后政策。DShop **不感知** PiEcho 内部实现（LLM、向量库、会话编排与 DShop 无关） |

**边界一句话**：PiEcho 是 DShop 的只读服务消费者，与 C 端商城、平台后台、商户后台并列为第 4 类调用方，但权限最小、能力最少、影响面最窄。
...
| 平台后台 | 类目、商品、订单、售后介入、营销、会员、内容位、结算、系统管理 |
| 商户后台 | 商品、SKU、库存、订单发货、售后处理、优惠券、店员、结算查询 |
| API | 唯一后端（BFF）：`/shop`、`/admin`、`/merchant`、`/callbacks`、`/agent` 五组路由 |
| **Agent 只读接口** | `/api/v1/agent/...` 六端点 + 服务令牌体系 + 脱敏 + 限流 + 契约版本化（**M0 交付**，见 §12） |
...
- 供 PiEcho **写入**的任何接口（Agent 组路由**只允许 GET**，见 §7.8.3）
...
| 限流 | **应用层自研**（Hono 中间件 + **Cache API 固定窗口计数**，可选 **Durable Object** 收敛为全局硬配额），Agent 组独立配额 | 免费层即可用且不消耗 KV 写配额（§7.8.4、§10.1）；**注意 Cache API 计数为 per-colo 近似值**，需要全局精确配额时启用 DO | WAF 限流（Dashboard 配置） |
...
**A4 的用途（本量级是下列取值的直接依据，不是背景描述）**：

| 取值 | 依据 |
| --- | --- |
| **Agent 限流配额**（令牌全局 600 次/分钟、订单类 120、商品/库存类 300、政策类 60，§7.8.4） | 按「日订单 < 1 万」推算客服会话量与查询放大倍数，保证 Agent 流量峰值不挤占商城主链路 |
...
| # | 待确认项 | 影响 | 需定案的时间点 |
| --- | --- | --- | --- |
| Q1 | **域名与 ICP 备案的主体及责任人** | 决定 `api.dshop.example.com` 等自定义域能否绑定（`*.workers.dev` 大陆不可达）。备案未完成则 Agent 接口在大陆不可达，PiEcho 无法调用，M4 验收不成立；同时阻塞二期小程序（request 合法域名要求 HTTPS + 备案）。**责任人：项目负责人牵头，平台主体提供资质** | **M0 启动，M4 前完成**（§11.3、§12） |
...
| D1 | `AGENT_DB` | api（**预留，本期不配**） | 升级缝 S11：D1 只读副本 / Sessions API，专供 Agent 组读路径 |
...
**编号约定**：**S1–S9 沿用通用商城（`eshop`）的原始编号**，逐条一一对应，便于两份文档互相对照与评审（特别地：**S7 是缓存、S8 是限流、S9 是出海/多区、S3b 是 CPU 密集任务**）；DShop 相对通用商城**新增的两条缝排在末尾，用 S10 / S11**，不复用既有编号。
...
| **S8** | **限流** | 应用层自研（Hono 中间件 + **Cache API 固定窗口计数**，不写 KV） | WAF 自定义限流规则；极端并发下可把计数器收敛到 **Durable Object**（免费层即可用，见下） | Dashboard 平台配置，应用层保留兜底 |
| **S9** | **出海 / 多区** | 单区域 D1 | 多区域副本 + 地理路由 | 同 S2 + Cloudflare 地理路由（**远期选项**） |
| **S10** | **Agent API 部署形态** | 与 `dshop-api` 同 Worker（同代码库、独立路由与中间件链） | 独立 Worker `dshop-agent-api` | 新增 Worker 配置 + 复制 `DB`/`KV` 绑定，代码复用 `packages/services` |
| **S11** | **Agent 读扩展** | 复用 `DB` | `AGENT_DB` 只读副本 | Agent 组读客户端按 `env.AGENT_DB` 是否存在自动选择 |

> S10/S11 是 DShop 相对通用商城的额外两条缝——唯一动机是**把 Agent 流量对商城主链路的影响降到零**：Agent 流量突发时，先靠 S11 隔离读，再靠 S10 隔离部署与配额。
...
| 域 | 表 | 关键字段 / 说明 |
| --- | --- | --- |
| **会员** | `users` | `phone`（唯一，加密存储）、`nickname`、`avatar_url`、`status`、`wechat_openid`（预留） |
...
| | `products` (SPU) | `merchant_id`、`category_id`、标题/副标题/主图、`detail_html`、`brand`、`status(draft/pending_review/onsale/offsale/rejected)` |
| | `product_skus` | `product_id`、`spec`(jsonb，如 `{"颜色":"黑","容量":"256G"}`)、`sku_code`、`price`、`stock`、`locked_stock`、`status(active/inactive)` |
| | **`product_attrs`** | ★ SPU 级参数白皮书：`spu_id`、`group_name`（如"基本信息"/"保修"）、`attr_name`、`attr_value`、`unit`、`sort_order`、`searchable` |
| | `product_images` | 图集，`sort_order` |
| **交易** | `cart_items` | `user_id`、`sku_id`、`quantity`、`checked`、`merchant_id`（冗余，供结算页分商户展示） |
| | `orders`（主单） | `order_no`（唯一）、`user_id`、`status`、`total_amount`、`discount_amount`、`freight_amount`、`pay_amount`、`address_snapshot`(jsonb)、`coupon_id`、`channel(web/miniprogram/app)`、`pay_deadline`、支付/完成时间 |
| | `sub_orders`（子单） | `sub_order_no`（唯一）、`order_id`、`merchant_id`、`store_id`、`status`、`subtotal`、`discount_alloc`、`freight`、`commission_amount`、`express_company`、`express_no`、`shipped_at`、`settled` |
| | `order_items` | `sub_order_id`、`sku_id`、**商品快照**（`title`/`image`/`spec`/`unit_price`）、`quantity`、`subtotal` |
| | `order_status_logs` | 状态流转：`from`/`to`、操作者类型/ID、备注、时间 |
...
| **售后** | `aftersales` | `aftersale_no`（唯一）、`sub_order_id`、`user_id`、`type(refund_only/return_refund)`、`status`、`reason`、`evidence_urls`(jsonb)、`refund_amount`、`return_express_no`、`deadline_at` |
| | `aftersale_logs` | 售后状态流转与操作者记录（客服介入留痕） |
| | **`aftersale_policies`** | ★ 售后政策条款：`category`（`return`/`refund`/`exchange`/`freight`/`warranty`）、`title`、`content`(markdown)、`version`、`effective_from`、`status` |
...
| | **`agent_call_logs`** | ★ Agent 调用审计：`token_id`、`path`、`params_hash`、`status`、`duration_ms`、`cache_hit`、`contract_version`、`created_at` |
...
**① 主单 + 子单拆单模型**：一次结算含多商户商品时，生成 **1 个主单**（用户视角、一次支付、一个 `order_no`）+ **N 个子单**（商户视角，独立发货、独立售后、独立结算，各自 `sub_order_no`）。优惠券与运费在主单层计算后**按子单 `subtotal` 比例分摊**，写入 `sub_orders.discount_alloc`；`order_items` 落快照。用户侧只感知主单状态，商户侧只感知自己的子单。**Agent 接口必须同时返回主单与子单**（客服要回答"其中一个包裹还没发"，见 §7.2）。
...
/api/v1/agent/...       # ★ 供 PiEcho 的只读接口（服务令牌，GET-only）
```
...
## 7. ★ 供 PiEcho 的只读 Agent API 契约

> 本章是 DShop ↔ PiEcho 的**唯一正式接口契约**。机器可读定义位于 `packages/shared/src/contracts/agent/`（Zod Schema），CI 生成 `docs/agent-api.openapi.json` 供 PiEcho 侧代码生成。**文档与 Schema 不一致时以 Schema 为准。**
...
**实现约束**：Agent 组**所有响应必须经过 `maskAgentPayload()` 单一出口函数**；该函数以 Zod Schema 的 `.strip()` 模式运行——**白名单外字段一律丢弃**，而非黑名单式删除。这是防止"新增字段意外泄露"的结构性保证。

#### 7.8.3 只读保证（四重）

| 层 | 机制 |
| --- | --- |
| ① HTTP 方法白名单 | Agent 路由组注册时**只挂 `GET`**；中间件 `agentReadOnlyGuard` 对 `POST/PUT/PATCH/DELETE` 一律返回 `405`。**不存在任何写端点** |
...
| ④ 部署级隔离 | 量级上来后拆独立 Worker + 只读副本（S10/S11）。**D1 无独立数据库账号概念**，故第 ④ 层是"用副本/独立部署代替只读账号"的等价方案 |
...
| 维度 | 默认配额 | 说明 |
| --- | --- | --- |
| 令牌全局 | `rate_limit_per_min`（默认 **600** 次/分钟） | 固定窗口计数**不逐请求写 KV**——默认由 **Cache API**（`caches.default`）承载，key = `agent:rl:{tokenId}:{window}`，每请求一次 Cache 读 + 一次 Cache 写（不计入 KV 写配额，也不消耗 D1 写）。⚠️ **Cache API 是 per-colo 且可被淘汰的**：同一令牌在 N 个 colo 上各可跑满 600/min，实际放行量约为 **600×N**，计数也可能随缓存淘汰丢失。因此 **600/min 是「单 colo 近似配额」，不是全局硬配额**。若需要**全局精确配额**，把计数器收敛到**单个 Durable Object 实例**（强一致、无写放大，免费层即可用，随 S8 打开）；**生产建议对 Agent 组启用 DO 计数**，Cache API 作为降级路径 |
| 单端点 | 见各端点「鉴权/限流」行 | 端点级配额更严，防止单点打爆 |
| 突发 | burst 20（令牌桶） | 应用层实现：同窗口计数器（Cache API / DO）判定 + 隔离实例本地内存平滑，**不写 KV** |
...
**履约分配（自营）**：下单后为自营子单分配履约门店——按收货地址与门店位置/库存就近分配（一期：优先级 + 后台手动改派）；用户自提时在结算页选店。平台化模式下 vendor 子单固定为其唯一门店（仓库），无分配环节。分配结果写 `sub_orders.store_id`，**也是 Agent 库存接口 `shipFrom` 的数据来源**。

**结算（平台化模式）**：Cron 按 T+N 账期扫描各 `vendor` 商户已完成子单 → 生成结算单（销售额 − 佣金 − 退款差额 = 应结金额）→ 平台财务确认 → 打款并标记已付。自营模式无结算流程（平台直收），仅提供经营报表；资金合规见 §11.2。
...
| 优惠券过期 | Cron 每日 | 批量置失效 |
| 物流轨迹同步 | Cron 每小时 | 拉取在途子单轨迹落库，供 Agent 接口读取 |
| `agent_call_logs` 落库 | Cron 每分钟 | 批量写入 Agent 调用审计 |
...
- 权限点命名 `域:动作`：`merchant:approve`、`product:review`、`order:ship`、`aftersale:approve`、`settlement:confirm`、`agent:token:manage`。
...
- 角色矩阵：顾客（下单/售后/评价）、平台超管（全部，含 `agent:token:manage`）、平台运营（类目/商品审核/营销/内容位）、平台财务（结算确认/对账/退款复核）、平台客服（订单查询/售后介入，不可改商品与资金配置）、商户管理员（本商户全部）、商户店员（发货/售后处理，无改价与店员管理）、**PiEcho Agent 服务身份（`/agent/*` 只读，无任何写权限）**。
...
# 5) 绑定自定义域（必须，*.workers.dev 大陆不可达）
#    Dashboard → Workers → <各 Worker> → Settings → Domains & Routes
#    dshop-api        → api.dshop.example.com
...
# 6) 签发 PiEcho 服务令牌（后台操作）
#    POST /api/v1/admin/agent-tokens { name:"piecho-prod", scopes:[...], expiresIn:180d }
```

**发布前 checklist**：

- [ ] `wrangler.json[c]` 中所有 `PLACEHOLDER_LOCAL_DEV` 已替换为真实实例 ID
- [ ] `JWT_SECRET` 与 `AGENT_TOKEN_PEPPER` 已 `wrangler secret put`（勿用 dev 默认值）
- [ ] seed 账号 `admin` / `selfshop` 已改密
- [ ] 自定义域已绑定且大陆可达性已实测
- [ ] PiEcho 服务令牌已签发并交付（仅一次性明文）
- [ ] **域名注册 + ICP 备案已完成**，自定义域生效且大陆可达性已实测（责任人：项目负责人，§11.3）
...
2. **Agent 契约链路**：签发测试令牌 → 调 6 个 Agent 端点 → 断言响应结构与脱敏规则 → 断言 `POST /agent/*` 返回 `405` → 断言无令牌返回 `40101`
...
- **Agent 面额外加固**：令牌仅存哈希、可立即吊销、可选请求签名防重放、响应白名单裁剪、独立限流配额、全量调用审计。
...
- 小程序 request 合法域名要求 **HTTPS + ICP 备案**——二期启动前必须完成备案。
- **PiEcho 侧同受此影响**：PiEcho 是 Node.js 服务，若部署在大陆境内，访问 `api.dshop.example.com` 走公网；若 DShop 仅用 `*.workers.dev`，PiEcho 将完全无法调用。**自定义域是 Agent 接口的前置条件**。
- **责任人与时间点**：域名注册 + ICP 备案由**项目负责人**牵头（法务/主体资质由平台主体提供），**M0 启动、M4 前完成**——M4 验收要求「Agent 接口在大陆网络实测可用」，该验收依赖备案完成后的自定义域生效（§12）。
...
> 一期上线**自营多门店**商城（M0–M4），并**在 M0 即交付供 PiEcho 调用的 Agent 只读接口**——PiEcho 的开发与联调不能被 DShop 的商品/交易域进度阻塞，且 Agent 接口依赖的表（订单/子单/物流/商品/售后）在 M0 的全量迁移中即已建好。**排期互锁约定**：M0 交付契约（OpenAPI + fixture 包）作为**交付物**，供 PiEcho 侧**离线**完成契约解析与集成验证（用途说明，非 DShop 的验收门禁）；**联调节奏以 DShop 里程碑为准——首次双端联调在 DShop M2（真实令牌 + 真实订单），生产联调在 M4（生产环境 + 大陆实测）**；PiEcho 侧自身的排期编号（其 M5）与 DShop 的 M0–M5 无对应关系，不作为 DShop 任何阶段的门禁。平台化入驻/抽佣/结算作为 M5 打开。

| 阶段 | 内容 | 验收标准 |
| --- | --- | --- |
| **M0 基础设施 + Agent 接口（2-3 周）** | Monorepo 脚手架（pnpm + Turbo + strict TS）、CI/CD 流水线、**D1 schema 全量迁移（41 表，含双模式全部表）**、三环境部署、`shared` 契约层、后台登录 + RBAC 骨架、seed（自营商户 + 首批门店 + 超管）；**★ 交付 `/api/v1/agent/*` 六端点 + `service_tokens` 令牌体系 + 脱敏器 + 限流 + 契约版本化 + OpenAPI 导出，并交付契约 + OpenAPI + fixture 包**（fixture 包的用途：供 PiEcho 侧离线完成契约解析与集成验证，不作为 DShop 的验收条件）；并行：提交微信商户号 + 支付宝直签申请、预研电商收付通资质、**启动域名注册与 ICP 备案（责任人：项目负责人，M4 前完成）** | **DShop 自证**：CI 全绿；staging 可登录三入口；迁移可回滚；**契约文档 + OpenAPI + fixture 包已交付，且 DShop 侧契约快照测试通过（`pnpm test --filter @dshop/shared`）**；脱敏断言与 `POST /agent/*` 返回 `405` 的只读断言通过 |
| **M1 商品域（2-3 周）** | 平台：类目、商品管理、`product_attrs` 参数维护（**同时喂给 Agent `/specs`**）、审核位预留；商户：商品/SKU/库存/运费模板；C 端：首页/分类/详情（SSR + Cache API） | 自营商品可上架，C 端可浏览详情；**Agent `/specs` 与 `/stock` 返回真实上架商品数据** |
| **M2 交易域（2-3 周）** | 购物车、下单拆单、库存原子锁定、**普通商户号直收**（微信 + 支付宝沙箱）、订单列表、履约分配与门店发货、超时关单、**物流轨迹同步任务** | E2E 主链路通过；压测下单接口不超卖；**Agent `/orders/{orderNo}` 返回真实子单与物流轨迹**；**与 PiEcho 完成首次双端联调（真实令牌 + 真实订单）** |
| **M3 营销与售后（2 周）** | 优惠券、满减、评价；售后申请/退货退款全流程；客服介入；**`aftersale_policies` 政策发布流程** | 售后全流程 + 优惠金额分摊正确；**Agent `/aftersales/{no}` 与 `/policies/{category}` 全状态覆盖** |
| **M4 运营与上线（2 周）** | 工作台看板、经营报表（按门店维度）、审计日志查询、**Agent 配额看板（`agent_call_logs` 聚合）**、安全加固（TOTP 强制、WAF 规则、压测） | 生产上线自营多门店商城；**Agent 接口在大陆网络实测可用，P95 < 300ms**；**完成与 PiEcho 的生产双端联调（真实令牌 + 真实订单 + 契约版本核对）** |
| **M5 平台化（后续）** | 商户入驻流程、资质审核、商品平台审核（对 vendor）、抽佣配置、分账结算（电商收付通/支付宝分账）、门店级库存（按需） | 第三方商户可入驻至结算全流程闭环 |

每阶段结束做一次回归 E2E（含 Agent 契约链路）+ 数据库迁移演练。
...
| # | 风险 | 等级 | 缓解措施 |
| --- | --- | --- | --- |
| R1 | **大陆用户访问 Cloudflare 延迟/稳定性不足** | 高 | 绑定自定义域（`*.workers.dev` 大陆不可达）；M0 内完成大陆实测；启动 ICP 备案；保留迁移路径（Hono/Drizzle 可脱离 CF，§11.3） |
| R2 | **D1 写入吞吐上限**（单库、无交互事务） | 中 | 单语句原子 UPDATE + `DB.batch` 打包；`task_queue` 削峰；读路径以 Cache API 为主；M2 压测设定容量红线；`agent_call_logs` 异步批量落库 |
| R3 | **二清合规风险**（平台代收再结算） | 高 | 一期自营直收**完全规避**；M5 平台化前必须落地分账方案（资质 M0 起并行申请）；资质未获批则平台化不上线 |
...
| R6 | **Agent 流量突发影响商城主链路** | 中 | 独立路由组 + 独立令牌配额 + 端点级限流；S11（`AGENT_DB` 只读副本）与 S10（独立 Worker）已预留；高频端点边缘缓存 |
| R7 | **Agent 接口数据泄露（隐私）** | 高 | 响应白名单裁剪（Zod `.strip()`）+ 单一脱敏出口；手机号/地址强制脱敏；凭证图与完整地址不下发；全量调用审计；令牌最小 scope |
| R8 | **契约破坏性变更导致 PiEcho 线上故障** | 中 | `X-Contract-Version` 版本化 + 双版本并行 ≥ 90 天；CI 契约快照测试拦截 breaking change；PiEcho 用生成的类型化客户端，不兼容即编译失败 |
...
| R11 | 免费层额度耗尽影响开发 | 低 | 免费层仅用于 dev/preview；**preview 禁止压测**，压测一律在 staging（付费层）执行 |
...
| 声明 | 含义 |
| --- | --- |
| **DShop 不感知 PiEcho 内部实现** | DShop 不知道 PiEcho 用哪个 LLM、用 LanceDB 还是别的向量库、如何编排会话。DShop 只暴露 §7 的只读 HTTP 契约 |
...
| **DShop 不接收 PiEcho 的写入** | Agent 路由组 GET-only，无任何写端点（§7.8.3）。PiEcho 的"业务动作"（改地址、取消订单）必须回到商城自身的用户界面 |
| **PiEcho 不直连 D1** | 唯一通道是 HTTPS + 服务令牌；PiEcho 侧**没有** D1 凭据，也无法通过任何 DShop 接口获得写能力 |
| **单向依赖** | 依赖方向恒为 `PiEcho → DShop`；DShop 的发布不依赖 PiEcho 的任何状态，PiEcho 宕机不影响 DShop 任何功能 |
...
| C5 | 域名与网络可达性 | `api.dshop.example.com` 自定义域 | 域名变更需提前 30 天通知 |

**除 C1–C5 外，双方无任何共享代码、共享数据库、共享部署流水线。**
...
2. CI：生成 OpenAPI + 契约快照 → breaking-change 检测
...
| 场景 | 约定 |
| --- | --- |
| DShop Agent 接口不可用 | PiEcho 必须降级为"暂时查询不到，请稍后再试"话术，**不得**编造订单/物流/售后信息 |
...
| 未覆盖的问题类型 | PiEcho 应转人工，**不得**猜测；DShop 按客服反馈评估新增端点（走 §7.9 变更流程） |

### 14.5 后续演进（本期不做）

| 演进项 | 说明 |
| --- | --- |
| Agent API 独立 Worker（S10） | 流量占比超阈值时拆分，独立配额、独立发布节奏 |
| `AGENT_DB` 只读副本（S11） | Agent 读流量与商城读流量物理隔离 |
| Agent 端点扩展 | 按客服高频问题补充：「按物流单号查订单」「按手机号查售后单」「商品 FAQ」 |
| Webhook 推送（可选） | 订单/售后状态变更时主动推送给 PiEcho，替代部分轮询——需评估接收端与重试语义，**本期明确不做** |
| 批量接口（可选） | `GET /agent/orders/batch?orderNos=` 降低会话内多次查询的往返开销 |
