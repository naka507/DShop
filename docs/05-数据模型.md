...
**三处口径必须一致**：下单成功 = 仅 `locked_stock` 增 q（可售库存 −q）；支付成功 = `stock` 减 q **且** `locked_stock` 减 q（可售库存不变）；超时关单 / 用户取消 = 仅 `locked_stock` 减 q（可售库存 +q）。**禁止写法**：下单时同时 `stock = stock - ?` 与 `locked_stock = locked_stock + ?`——那会让可售库存虚减 2q，且 `WHERE stock >= ?` 的判据与实际可售口径不符（少卖）。一个主单的「锁定全部 SKU + 写主单 + 写子单 + 写 `order_items` + 写状态日志」用 **`DB.batch()`** 打包为原子批次。

> ⚠️ **`batch()` 的失败语义（易错点，`repositories/shop-orders.ts` 文件头有同样登记）**：D1 的 `batch()` **只在「语句报错」时整批回滚**；**`changes = 0` 不会触发回滚**——它是「该语句的 `WHERE` 没命中」的静默空转。本仓的多处原子迁移**正依赖**这一语义：例如 `cancelUnpaidOrder` 把「子单同步 → 状态日志 → 逐 SKU 释放锁定」的守卫写成 `EXISTS (... status = 'PENDING_PAYMENT')`，并让**状态迁移语句排在最后**——重复调用时前三条因守卫不成立而空转（`changes = 0`，**不回滚**），末条也 `changes = 0`，于是返回值正确表示「本次未迁移」。若误以为 `changes = 0` 会回滚，就会写出错误的幂等逻辑。
