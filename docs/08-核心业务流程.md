...
| 超时未支付关单                     | Cron 每分钟 | 生产者在下单成功后入队 `order.timeout_cancel` 并带 `delaySeconds` = 支付期限剩余秒数（`task_queue.run_at` / Queues `delaySeconds` 据此推迟）；消费者（Cron 轮询 / Queues 出口）**二次校验 `orders.pay_deadline`**：未到期则把任务延后到该时刻、**绝不关单**，已到期才关单 + 仅释放锁定（`locked_stock -= q`，不动物理 `stock`，§5.3 ②）。**没有独立的 `pay_deadline` 扫描器**——关单判据只有这条任务 |
...
| task_queue 消费                    | Cron 每分钟 | 按类型分发，单次最多 N=50 条；`processing` 超时重回 `pending`（幂等），`attempts >= TASK_MAX_ATTEMPTS`（=5，代码常量；表无此列）置 `failed` 进死信可重放；失败重试按 `run_at = now + min(60·2^(attempts-1), 3600)` 秒有界退避 |
