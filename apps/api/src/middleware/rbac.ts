...
import { AGENT_ERROR_CODES, roleHasPermission } from "@dshop/shared";
...
export const requirePermission = (
  ...
  if (subject === undefined) {
    return errorResponse(AGENT_ERROR_CODES.SCOPE_INSUFFICIENT, "未登录");
  }
...
/** 要求指定角色。 */
export const requireRole = (roleCode: string): MiddlewareHandler<AppEnv> => async (
  c,
  next,
) => {
  const subject = c.get("adminSubject");
  ...
  if (subject.role !== roleCode) {
    return errorResponse(AGENT_ERROR_CODES.SCOPE_INSUFFICIENT, `需要角色：${roleCode}`);
  }
  await next();
};
